...
#   interface "#{node['cepheus']['network']['public']['interface']}"
#   permanent true
# end

# firewall_rule 'cluster' do
#   interface "#{node['cepheus']['network']['cluster']['interface']}"
#   command  :allow
# end
...
if node['cepheus']['security']['firewall']['enable']
  include_recipe 'cepheus::firewall-start'
  ...
  include_recipe 'cepheus::firewall-reload'

  bash 'set-default-zone' do
    user 'root'
    code <<-EOH
      firewall-cmd --set-default-zone=#{node['cepheus']['security']['firewall']['zone']}
    EOH
  end
  ...
  ruby_block 'force later execution' do
    block do
      if node['cepheus']['security']['firewall']['use'] == 'rules'
        node['cepheus']['security']['firewall']['rules'].each do | rule |
          allow = false
        ...
        include_recipe 'cepheus::firewall-rules-interfaces'
      end
    end
  end
