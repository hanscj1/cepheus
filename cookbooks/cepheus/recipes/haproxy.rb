...
bash 'copy-ssl-certs' do
  user 'root'
  code <<-EOH
    sudo cp /tmp/*.pem #{node['cepheus']['adc']['ssl']['path']}/.
    sudo cp /tmp/*.crt #{node['cepheus']['adc']['ssl']['path']}/.
    sudo cp /tmp/*.key #{node['cepheus']['adc']['ssl']['path']}/.
    sudo chmod 0444 #{node['cepheus']['adc']['ssl']['path']}/*
  EOH
  ignore_failure true
end
...
node['cepheus']['adc']['vips'].each do | vip |
  if vip['ssl'] == true && !File.exists?("#{vip['cert']}")
    execute 'dev-null-cert' do
     command lazy { "cp /dev/null #{node['cepheus']['adc']['ssl']['path']}/#{vip['cert']}" }
    end
    ...
    vip['ssl_files'].each do | ssl_file |
      bash "build-ssl-cert-#{vip['name']}" do
        user 'root'
        code <<-EOH
          if [[ #{node['cepheus']['adc']['ssl']['path']}/#{ssl_file} != #{node['cepheus']['adc']['ssl']['path']}/#{vip['cert']} ]]; then
            cat #{node['cepheus']['adc']['ssl']['path']}/#{ssl_file} >> #{node['cepheus']['adc']['ssl']['path']}/#{vip['cert']}
          fi
        EOH
      end
    end
    ...
    execute 'chmod-cert' do
     command lazy { "chmod 0444 #{node['cepheus']['adc']['ssl']['path']}/#{vip['cert']}" }
    end
  end
end
# SSL End
