...
def init_config
    ...
    begin
        ...
        $edbi = Chef::EncryptedDataBagItem.load('configs', node.chef_environment) if node['cepheus']['enabled']['encrypt_data_bag']
...
def set_item(key, value, force=false)
    ...
    if $dbi[key].nil? or force
        $dbi[key] = (node['cepheus']['enabled']['encrypt_data_bag']) ? Chef::EncryptedDataBagItem.encrypt_value(value, Chef::EncryptedDataBagItem.load_secret) : value
        $dbi.save
        $edbi = Chef::EncryptedDataBagItem.load('configs', node.chef_environment) if node['cepheus']['enabled']['encrypt_data_bag']
        ...
        return (node['cepheus']['enabled']['encrypt_data_bag']) ? $edbi[key] : $dbi[key]
    end
end
...
def is_defined(key)
    ...
    result = (node['cepheus']['enabled']['encrypt_data_bag']) ? $edbi[key] : $dbi[key]
    return !result.nil?
end
...
def is_bootstrap_node
  val = false
  if node['hostname'] == node['cepheus']['bootstrap']['name']
    val = true
  end
  val
end
...
def get_ip(interface)
  ...
  if is_bootstrap_node
    node['cepheus']['bootstrap']['interfaces'].each do | intf |
      if interface == intf
        val = intf['ip']
        break
      end
...
def adc_nodes
  results = search(:node, "tags:#{node['cepheus']['adc']['tag']}")
  results.map! { |x| x['hostname'] == node['hostname'] ? node : x }
  if !results.include?(node) && node.run_list.roles.include?(node['cepheus']['adc']['tag'])
    results.push(node)
  end
...
def is_adc_node_role(role)
  ...
  if is_adc_node
    node['cepheus']['adc']['bgp']['roles'].each do |n|
...
def get_backend_int_attr(name, attr)
  val = 0
  backend_nodes = node['cepheus']['adc']['backend']['servers']
