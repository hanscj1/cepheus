...
do_on_node $CEPH_CHEF_BOOTSTRAP "cd \$HOME && rsync -a /ceph-host/* ./cepheus"

# add the dependency cookbooks from the file cache
echo "Checking on dependency for cookbooks..."
do_on_node $CEPH_CHEF_BOOTSTRAP "cp /ceph-files/cookbooks/*.tar.gz \$HOME/cepheus/cookbooks"
do_on_node $CEPH_CHEF_BOOTSTRAP "cd \$HOME/cepheus/cookbooks && ls -1rtc *.tar.gz | xargs -t -I% tar xvzf %"
do_on_node $CEPH_CHEF_BOOTSTRAP "cd \$HOME/cepheus/cookbooks && rm -f *.tar.gz"
